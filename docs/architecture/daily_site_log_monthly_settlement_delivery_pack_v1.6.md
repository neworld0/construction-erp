# [DAILY-SITE-LOG-MONTHLY-SETTLEMENT-01] 설계·전환 Delivery Pack v1.6

> **v1.6 PDF/인쇄 게이트:** FIELD·HQ·CEO 범위별 버튼 노출, 다운로드 RBAC 재검증, PDF `application/pdf` 응답, 한글 제목·기간별 표·업무 내역의 렌더링, 원천 데이터 무변경을 통과 기준으로 한다.

> **v1.5 추가 게이트:** 단일 프로젝트 보고서와 법인/그룹 종합보고서가 전일·금일·당월 누계 및 금일 업무 내역을 표시하는지, 세 기간 합계가 원천과 같은지, 빈 장비·물량 항목을 허위 값으로 만들지 않는지 확인한다.
...
> **v1.4 구현 제약:** FIELD `보고서` 메뉴·작성 CTA·모바일 진입을 추가하거나 복구하지 않는다. 기존 `FieldReport`는 읽기 전용 이력·미결 승인 처리 외에는 신규 업무 경로에서 제외한다.
