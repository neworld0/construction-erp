# 공사일보·월 정산 확장 설계 정밀 검증 보고서 v1.6

> **v1.6 검증 범위:** 출력은 화면 보고서의 대체 입력물이 아니라 같은 파생 집계의 표현 형식이다. 권한 없는 직접 URL 접근, 다른 법인/프로젝트 PDF 조회, 빈 범위의 PDF 생성, 다프로젝트 업무 내역 줄바꿈을 검증 대상으로 추가한다.
