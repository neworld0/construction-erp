# 공사일보·월 정산 확장 논리 ERD v1.6

> **v1.6 출력 파생물:** `ProjectDailyReportPdf`와 `OrganizationDailyReportPdf`는 각각 대응하는 읽기 뷰의 일회성 표현이며 저장 엔터티가 아니다. 원천 행·스냅샷·승인 객체를 생성하거나 변경하지 않는다.
