# 공사일보·월 정산 확장 물리 ERD v1.6

> **v1.6 물리 제약:** PDF는 서버 메모리 스트림으로 생성·응답하고 별도 파일/테이블/마이그레이션을 만들지 않는다. 다운로드 URL은 화면 조회와 동일한 프로젝트 접근 및 법인 범위 검사를 통과해야 한다.

> **v1.5 물리 구현:** 전일·금일·월누계 및 업무 내역은 `schedule_dailyprogress`, `labor_timesheet(+line)`, `cost_costactual(+line)`, `inventory_issuetowork(+line)`, `inventory_inventoryledger`의 프로젝트·일자 인덱스를 이용한 읽기 집계다. 새 테이블·마이그레이션은 추가하지 않는다.
