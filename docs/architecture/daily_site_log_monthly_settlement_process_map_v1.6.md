# 공사일보·월 정산 확장 프로세스맵 v1.6

> **v1.6 출력 흐름:** FIELD 프로젝트 공사일보, HQ 법인/프로젝트 공사일보, CEO 법인/그룹 공사일보에서 같은 기준일·범위로 화면 인쇄 또는 PDF를 생성한다. 출력은 읽기 전용이며 원천·승인·마감 상태와 데이터는 변경하지 않는다.

> **v1.5 보고서 형식:** `프로젝트 공사일보`는 기준일의 전일·금일·당월 누계를 나란히 표시하고, 금일의 진행률·출역·원가·자재투입 원천에서 업무 내역을 재현한다. `법인/그룹 공사일보`는 같은 기준으로 여러 프로젝트를 한 장의 관리 보고서에 취합하고, 각 프로젝트 상세로 드릴다운한다.
